![Banner de portada del curso](../img/portada.png)

# L2 · *Tall og tid* — Números, días y el calendario noruego
**Tiempo estimado: 30 minutos**
 
Acabas de aprender a presentarte. La pregunta natural que sigue en cualquier conversación es **«¿cuántos años tienes?»**, **«¿qué día es?»** o **«¿cuándo es tu cumpleaños?»**. Para responder a todo eso necesitas tres cosas: números, días y meses. Vamos a por ellas.
 
Pero esta lección no va solo a darte una lista. Los noruegos viven su año marcado por unas pocas fechas: el ***17. mai*** (el día nacional), el ***jul*** (la Navidad noruega) y los **meses sin sol** del invierno polar, que en el norte de Noruega son literales.
 
Al terminar esta lección, serás capaz de:
 
- **Contar** del 0 al 100 en noruego.
- **Decir y entender** los días de la semana y los meses del año.
- **Preguntar y dar fechas** sencillas: cumpleaños, hoy, mañana.
- **Reconocer** las festividades más importantes del calendario noruego y por qué son culturalmente clave.
- **Identificar** la diferencia entre la forma moderna de contar (post-1951) y la forma antigua que todavía oirás de gente mayor.
Esta lección tiene cinco actividades:
 
- 📖 Lectura guiada: números, días, meses y festividades (12 min)
- ✅ Cuestionario interactivo: números en contexto (4 min)
- ✏️ Mini-ejercicios de autocorrección (4 min)
- 📚 Anexo EPUB: *Det norske året* — el año noruego mes a mes (8 min)
- 💭 Reflexión: tu propio calendario (6 min)
Hazlas en orden. ¿Listo? *Vi setter i gang!*
 
---
 
## 📖 Números, días, meses y festividades
 
**Tiempo de lectura: 12 minutos**
 
### Antes de leer
 
En español, ¿qué hora son las 14:30? "Las dos y media de la tarde". ¿Y en formato 24h? "Las catorce treinta". Las dos son correctas y conviven. **En noruego pasa lo mismo con los números**: hay una forma moderna y una forma antigua, y vas a oír las dos. Conviene que las conozcas para no agobiarte cuando alguien diga 21 de una manera que no esperabas.
 
### Los números del 0 al 20
 
Empezamos por los básicos. Estos hay que **memorizarlos**: no se construyen con reglas, son palabras únicas.
 
| Cifra | Noruego | Cifra | Noruego |
|------|---------|------|---------|
| 0 | *null* | 11 | *elleve* |
| 1 | *en / ei / ett* | 12 | *tolv* |
| 2 | *to* | 13 | *tretten* |
| 3 | *tre* | 14 | *fjorten* |
| 4 | *fire* | 15 | *femten* |
| 5 | *fem* | 16 | *seksten* |
| 6 | *seks* | 17 | *sytten* |
| 7 | *sju* (o *syv*) | 18 | *atten* |
| 8 | *åtte* | 19 | *nitten* |
| 9 | *ni* | 20 | *tjue* (o *tyve*) |
| 10 | *ti* |  |  |
 
> 💡 **Truco**: mira *to, tre, fire, fem, seks, sju, ni, ti*. Compáralos con *two, three, four, five, six, seven, nine, ten*. Vienen de la misma raíz. Las únicas que cuesta un poco más son *åtte* (eight) y *elleve* (eleven), y aún así el parecido sigue ahí.
 
> ⚠️ **Una palabra para "uno", tres formas**: en noruego, *uno* tiene tres formas según el género del sustantivo que acompaña: *en gutt* (un chico), *ei jente* (una chica), *ett hus* (una casa). Suena raro al principio, pero solo afecta al número 1; el resto de números no cambian.
 
> 📌 **Forma moderna vs. antigua**: para el 7 puedes oír *sju* o *syv*; para el 20, *tjue* o *tyve*. En 1951 el Parlamento noruego adoptó *sju* y *tjue* como las formas oficiales modernas. *Syv* y *tyve* siguen siendo correctas y las usa mucha gente mayor o más conservadora. **Tú aprende y usa las modernas (*sju*, *tjue*)**; reconoce las antiguas cuando las oigas.
 
### Las decenas
 
Para llegar a 100, primero las decenas:
 
| Cifra | Noruego |
|------|---------|
| 20 | *tjue* |
| 30 | *tretti* |
| 40 | *førti* |
| 50 | *femti* |
| 60 | *seksti* |
| 70 | *sytti* |
| 80 | *åtti* |
| 90 | *nitti* |
| 100 | *(ett) hundre* |
 
> 💡 Otra vez el atajo germánico: *tretti* — *thirty*, *førti* — *forty*, *femti* — *fifty*, *sytti* — *seventy*... La regla es clara: la decena se forma con el número base + sufijo *-ti* (que viene de *ti* = diez).
 
### Los números compuestos: 21, 32, 67...
 
Aquí está la trampa del idioma. Hoy en día, la forma estándar es **decena + unidad**, todo junto, como en inglés:
 
| Cifra | Noruego moderno | Equivalente inglés |
|------|----------------|-------------------|
| 21 | *tjueen* | twenty-one |
| 32 | *trettito* | thirty-two |
| 47 | *førtisju* | forty-seven |
| 88 | *åttiåtte* | eighty-eight |
 
> 📌 **Ojo a la forma antigua**: hasta 1951, el orden era al revés (unidad + decena, como en alemán: *énogtyve* = "uno y veinte" = 21). Esta forma **todavía la oirás** de gente mayor, sobre todo al dar números de teléfono o precios. No te asustes si en una caja de supermercado oyes *énogtyve kroner* en vez de *tjueén kroner*: es lo mismo.
 
### Los días de la semana
 
Los noruegos empiezan la semana en **lunes**, no en domingo. Los nombres vienen, casi todos, de la mitología nórdica — **una buena razón para llegar bien a L6**:
 
| Día | Noruego | Origen |
|-----|---------|--------|
| Lunes | *mandag* | Día de la Luna (*Måne* = Luna) |
| Martes | *tirsdag* | Día de **Tyr** (dios nórdico) |
| Miércoles | *onsdag* | Día de **Odín** |
| Jueves | *torsdag* | Día de **Thor** |
| Viernes | *fredag* | Día de **Frigg** (diosa nórdica) |
| Sábado | *lørdag* | Día del baño (del nórdico antiguo *laug* = baño) |
| Domingo | *søndag* | Día del Sol (*Sol* = Sol) |
 
> 💡 **Truco**: el inglés tomó los mismos nombres mitológicos para sus días — *Tuesday* (Tyr), *Wednesday* (Wodan/Odín), *Thursday* (Thor), *Friday* (Frigg). Si te sabes los días en inglés, en noruego solo tienes que cambiar la terminación a *-dag* (que es "día" — como el alemán *Tag*).
 
### Los meses del año
 
Los meses son fáciles porque vienen del latín y se parecen mucho al español y al inglés:
 
| Mes | Noruego | Mes | Noruego |
|-----|---------|-----|---------|
| Enero | *januar* | Julio | *juli* |
| Febrero | *februar* | Agosto | *august* |
| Marzo | *mars* | Septiembre | *september* |
| Abril | *april* | Octubre | *oktober* |
| Mayo | *mai* | Noviembre | *november* |
| Junio | *juni* | Diciembre | *desember* |
 
> ⚠️ **Los meses van en minúscula** en noruego, igual que en español. Solo se ponen en mayúscula si están al principio de una frase. Es un error típico de hispanohablantes con buen inglés (donde sí se escriben con mayúscula): *I was born in May* → *Jeg er født i mai*.
 
### Preguntar y dar fechas
 
Cuatro fórmulas mínimas:
 
| Para preguntar / decir... | En noruego |
|---------------------------|------------|
| ¿Qué día es hoy? | *Hvilken dag er det i dag?* |
| Hoy es lunes. | *I dag er det mandag.* |
| ¿Cuándo es tu cumpleaños? | *Når har du bursdag?* |
| Mi cumpleaños es el 14 de marzo. | *Bursdagen min er den 14. mars.* |
 
> 📌 **El truco del punto**: ¿has visto el punto detrás del 14? En noruego, los números ordinales (1.º, 2.º, 14.º...) se escriben **con un punto detrás de la cifra**, no con superíndice. Por eso el día nacional se escribe ***17. mai*** y nunca *17º de mai*.
 
### Tres fechas para entender Noruega
 
Hay tres momentos del año que cualquier persona que se acerque a Noruega tiene que conocer.
 
**🇳🇴 *17. mai* — El día nacional (17 de mayo)**
 
Es la fecha más importante del calendario noruego. Conmemora la firma de la Constitución de 1814, cuando Noruega se declaró nación independiente (aunque siguió bajo unión con Suecia hasta 1905). El día se vive con **desfiles infantiles** (no militares, una rareza europea), todo el mundo viste el ***bunad*** (traje tradicional regional), las calles se llenan de banderas, se canta el himno nacional y se comen **hot dogs y helados sin medida**. Si alguna vez te invitan a un *17. mai*, no faltes: es la mejor lección de cultura noruega que puedes recibir.
 
**🎄 *Jul* — La Navidad noruega (diciembre y enero)**
 
*Jul* es la palabra noruega para "Navidad", pero abarca mucho más que el 25 de diciembre: empieza en torno al 13 de diciembre con ***Lucia*** (una tradición de origen sueco con desfile de niñas con coronas de velas) y se extiende hasta el 6 de enero. La cena principal es la ***julemiddag*** del 24, no del 25. Plato típico: ***ribbe*** (panza de cerdo asada) o ***pinnekjøtt*** (costillas de cordero saladas). Y hay un detalle único: muchas familias dejan un cuenco de gachas (***julegrøt***) para el ***fjøsnisse***, un duendecillo del granero que protege la casa si lo tratas bien.
 
**🌒 *Mørketid* — La temporada oscura (noviembre a enero)**
 
En el norte de Noruega (por encima del Círculo Polar Ártico, ciudades como **Tromsø**), el sol **no sale durante semanas** entre noviembre y enero. Es lo que se llama ***mørketid*** ("tiempo oscuro"). En Tromsø, el sol desaparece sobre el 27 de noviembre y vuelve sobre el 15 de enero — casi dos meses sin luz directa. La compensación: las **auroras boreales** son espectaculares. La cultura noruega del invierno (velas, *koselig*, esquí, sauna...) se entiende mejor cuando sabes que **el sol no es un dado**.
 
### Mientras lees, fija tres ideas
 
1. **Para contar del 21 al 99, decena primero y unidad después**, todo junto: *tjueen, trettito, førtitre...* Como en inglés.
2. **Los meses van en minúscula**. Importa: es un error que los anglohablantes cometen siempre.
3. **El calendario noruego no es neutro**: tres fechas (*17. mai*, *jul*, *mørketid*) explican una buena parte de cómo viven los noruegos su año.
---
 
## ✅ Cuestionario interactivo: números en contexto
 
**Tiempo estimado: 4 minutos**
 
Ahora vamos a comprobar si has interiorizado los números no sueltos, sino **en situaciones reales**: precios, edades, fechas, números de teléfono.
 
### Instrucciones
 
El cuestionario tiene 10 preguntas de tres tipos:
 
- **Opción múltiple**: lees una pregunta o escuchas un número y eliges la opción correcta.
- **Rellena el hueco**: escribes la palabra noruega correspondiente a un número.
- **Asociar**: emparejas números escritos con cifras.

Responde todas las preguntas y pulsa Enviar respuestas al final para ver tu puntuación y la explicación de cada pregunta.

➡️ **Abrir el cuestionario interactivo**: [Quiz de números noruegos](quiz-numeros/index.html ':ignore')
 
> 📌 **Nota técnica**: el cuestionario también está disponible en formato **IMS QTI 1.2**, un estándar interoperable que cualquier plataforma educativa (Moodle, Canvas, Blackboard…) puede importar. Puedes [descargar el paquete QTI](quiz-numeros.qti.zip ':ignore') o la [clave de corrección en PDF](quiz-numeros-clave.pdf ':ignore') si lo prefieres en papel.

---
 
## ✏️ Mini-ejercicios: comprueba lo que has aprendido
 
**Tiempo estimado: 4 minutos**
 
Estos ejercicios son **de autocorrección**: las respuestas están al final, justo debajo. Resuelve primero todo lo que puedas sin mirar, y solo después comprueba.
 
### Ejercicio 1 — Escribe los números en noruego
 
1. 14 → ______________________
2. 27 → ______________________
3. 41 → ______________________
4. 68 → ______________________
5. 99 → ______________________
### Ejercicio 2 — Traduce las fechas al noruego
 
1. Hoy es martes. → ______________________
2. Mi cumpleaños es el 3 de septiembre. → ______________________
3. ¿Qué día es hoy? → ______________________
4. Estamos en mayo. → ______________________ (pista: *Vi er i ...*)
### Ejercicio 3 — Conecta los días con su dios
 
| Día noruego | Origen mitológico |
|-------------|-------------------|
| 1. *torsdag* | a. Frigg |
| 2. *onsdag* | b. Thor |
| 3. *fredag* | c. Tyr |
| 4. *tirsdag* | d. Odín |
 
### Ejercicio 4 — Falso amigo o cognado
 
Mira estas palabras noruegas. Identifica si tienen cognado en inglés (y cuál es) o si no se parecen a nada conocido:
 
1. *vinter* → ______________________
2. *bursdag* → ______________________ (pista: *birthday*)
3. *dag* → ______________________
4. *uke* (semana) → ______________________
5. *år* → ______________________
---
 
<details>
<summary><strong>🔎 Mostrar respuestas</strong> (haz clic para desplegar)</summary>
**Ejercicio 1:**
 
1. *fjorten*
2. *tjuesju* (también vale *tjuesyv* en forma antigua)
3. *førtien*
4. *sekstiåtte*
5. *nittini*
**Ejercicio 2:**
 
1. *I dag er det tirsdag.*
2. *Bursdagen min er den 3. september.* (con el punto detrás del 3)
3. *Hvilken dag er det i dag?*
4. *Vi er i mai.* (en minúscula, sin punto antes ni después)
**Ejercicio 3:** 1-b (torsdag/Thor), 2-d (onsdag/Odín), 3-a (fredag/Frigg), 4-c (tirsdag/Tyr).
 
**Ejercicio 4:**
 
1. *vinter* → **winter** (cognado directo)
2. *bursdag* → **birthday** (cognado: *burs* + *dag*, "día de nacimiento")
3. *dag* → **day** (cognado, y también en alemán *Tag*)
4. *uke* → **week** (cognado un poco más lejano, pero misma raíz germánica: *uke*/*Woche*/*week*)
5. *år* → **year** (cognado: *år*/*year*/alemán *Jahr*)
</details>
> 💡 **Si has fallado más de la mitad de un ejercicio**: vuelve a las tablas de la lectura y repásalas. Especialmente la de números compuestos (21-99), que es la que más cuesta.
 
---
 
## 📚 Anexo EPUB: *Det norske året* — el año noruego mes a mes
 
**Tiempo estimado: 8 minutos**
 
Este anexo es una **lectura larga opcional pero recomendable**: un recorrido visual mes a mes por el calendario noruego, con fotos, datos curiosos y vocabulario añadido para cada mes. Está pensado para leerlo con calma en el móvil o en un lector electrónico, **no para resolver ejercicios**.
 
### Qué vas a encontrar
 
- 12 capítulos cortos (uno por mes) con:
  - El nombre del mes en noruego y la pronunciación.
  - Una fiesta o tradición típica.
  - Una palabra noruega "imposible de traducir" relacionada con ese mes (*koselig* para diciembre, *påskekrim* para abril, *grilling* para junio...).
  - Una imagen representativa.
- Glosario final con todas las palabras nuevas que aparezcan.
### Cómo abrirlo
 
- **En móvil**: descarga el archivo y ábrelo con cualquier app de lectura EPUB (Google Play Libros, Apple Libros, Moon+ Reader, Lithium...).
- **En ordenador**: usa Calibre, Adobe Digital Editions o Thorium.
➡️ **Descargar el anexo EPUB**: [det-norske-aret.epub](det-norske-aret.epub ':ignore')
 
---
 
## 💭 Reflexión: tu propio calendario
 
**Tiempo estimado: 6 minutos**
 
Para cerrar la lección, vamos a aterrizar lo aprendido en tu vida real. Escribe en una libreta o en un documento esta información sobre ti, **toda en noruego**:
 
1. *Bursdagen min er den... (tu fecha de cumpleaños).*
2. *I dag er det... (qué día y mes es hoy).*
3. *Jeg er ... år gammel (cuántos años tienes).*
4. *Yndlingsmåneden min er ... (tu mes favorito) fordi ... (en español o inglés, justificando).*
### Bonus pregunta cultural
 
5. ¿Cuál de las tres fechas noruegas que has visto (***17. mai***, ***jul***, ***mørketid***) te parece más diferente de cómo se vive en tu cultura? ¿Por qué?
Esta actividad no se evalúa. Sirve para **consolidar el vocabulario en algo que es tuyo**: tu propio cumpleaños se memoriza mucho mejor que el de un personaje inventado.
 
---
 
## Resumen de la lección
 
Si has llegado hasta aquí, ya sabes:
 
- ✅ Contar del 0 al 100 en noruego moderno (con guiño a la forma antigua).
- ✅ Los siete días de la semana y la mitología que está detrás.
- ✅ Los doce meses (en minúscula, recuerda).
- ✅ Cómo preguntar y dar una fecha sencilla.
- ✅ Por qué *17. mai*, *jul* y *mørketid* son culturalmente irrenunciables.
En la próxima lección sales del calendario y entras en el mapa: vas a aprender a moverte por Noruega, las preposiciones que te van a salvar la vida (*i*, *på*, *til*) y una narrativa interactiva donde tú eliges la ruta de tu propio viaje.
 
---
 
## Fuentes y créditos de esta lección
 
- **Språkrådet** (Consejo de la Lengua Noruega). *Tallord. Bokmål*. Recurso oficial sobre la reforma de 1951 y las formas modernas/antiguas de los números.
- **NDLA** (Nasjonal digital læringsarena). *Tidsuttrykk på norsk*. Recursos educativos oficiales del sistema escolar noruego.
- Faarlund, J. T., Lie, S., & Vannebo, K. I. (1997). *Norsk referansegrammatikk*. Universitetsforlaget. (Numeración, ordinales).
- **Visit Norway** (oficina oficial de turismo). *17. mai — Norway's Constitution Day* y *Christmas traditions in Norway*. Información cultural verificada.
- Datos sobre *mørketid* en Tromsø: **Norsk Polarinstitutt** (Instituto Polar Noruego).
